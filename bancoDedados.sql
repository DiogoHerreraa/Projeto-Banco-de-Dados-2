...
/*RELATORIO DE VENDAS FEITAS PELOS VENDEDORES*/
...
/*PROCEDURE DE REAJUSTE DE SALARIO*/
CREATE PROCEDURE REAJUSTAR_SALARIO (
    IN P_CATEGORIA ENUM('CEO','FINANCEIRO','VENDEDOR','ATENDENTE DE SUPORTE AO CLIENTE','GERENTE DE OPERAÇÕES'),
    IN P_PERCENTUAL FLOAT(5,2)
)
BEGIN
...
CALL REAJUSTAR_SALARIO('VENDEDOR', 10);


/*PROCEDURE RESPONSAVEL POR ATUALIZAR AS TABELAS DE VENDA ( QUEM VENDEU E QUEM COMPROU) E A TABELA DE ITENS VENDIDOS( O QUE COMPROU E QUANTO FOI)*/
DELIMITER $
...
BEGIN
    ...
    DECLARE V_SEXO ENUM('M','F','O');
    DECLARE V_IDADE INT;

   
    SELECT ID_CPADRAO, NOME_CPADRAO, SEXO, IDADE
    ...
    ELSE
        INSERT INTO CLIENTE_ESPECIAL
        VALUES (NULL, V_NOME_CLIENTE, V_SEXO, V_IDADE, 100, V_ID_CLIENTE);
    END IF;
    SELECT 
        C.NOME_CPADRAO AS CLIENTE_SORTEADO,
        E.CASHBACK
    FROM CLIENTE_PADRAO C
    LEFT JOIN CLIENTE_ESPECIAL E ON C.ID_CPADRAO = E.ID_CLIENTE
    WHERE C.ID_CPADRAO = V_ID_CLIENTE;
END
$
...
CALL REALIZAR_SORTEIO();


CREATE PROCEDURE REGISTRAR_VENDA (
    P_DATA_VENDA DATE, 
    P_ID_VENDEDOR INT,
    P_NOME_FUNCIONARIO VARCHAR(50),
    P_ID_CLIENTE INT,
    P_NOME_CLIENTE VARCHAR(50),
    P_ID_PRODUTO INT,
	P_NOME_PRODUTO VARCHAR(50),
    P_QUANTIDADE INT,
    P_VALOR_UNITARIO FLOAT(10,2)
)
BEGIN
    DECLARE V_ID_VENDA INT;
    DECLARE V_VALOR_TOTAL FLOAT(10,2);

    /*INSERIR NA TABELA VENDAS*/
    INSERT INTO VENDA 
    VALUES (NULL, P_DATA_VENDA, P_ID_VENDEDOR, P_NOME_FUNCIONARIO, P_ID_CLIENTE, P_NOME_CLIENTE);

    SET V_ID_VENDA = LAST_INSERT_ID();
    SET V_VALOR_TOTAL = P_QUANTIDADE * P_VALOR_UNITARIO;

    /*INSERIR NA TABELA INTENS VENDIDOS*/
    INSERT INTO ITEM_VENDA 
    VALUES (NULL, V_ID_VENDA, P_ID_PRODUTO,P_NOME_PRODUTO,P_QUANTIDADE, P_VALOR_UNITARIO, V_VALOR_TOTAL);
...
/*dados aleatorio para o teste do banco*/
CALL REGISTRAR_VENDA("2025-07-01", 3, 'WANDA', 15, 'FELIPE MORAES', 2, 'Teclado Mecânico', 1, 276.99);
CALL REGISTRAR_VENDA("2025-07-01", 4, 'WILLIAM', 18, 'FERNANDA', 5, 'Smartphone Android', 1, 2195.61);
CALL REGISTRAR_VENDA("2025-07-01", 5, 'KELLY', 21, 'RAFAEL', 10, 'Fone de Ouvido', 3, 189.94);
CALL REGISTRAR_VENDA("2025-07-01", 3, 'WANDA', 26, 'XIMENA', 4, 'Notebook i5', 1, 2064.47);
CALL REGISTRAR_VENDA("2025-07-01", 4, 'WILLIAM', 30, 'NEDSON', 12, 'Cabo HDMI 2.0', 2, 86.11);
...
/*Inserir funcionário especial se vender > R$1000 + bônus 5%*/


DELIMITER $$

CREATE TRIGGER trg_funcionario_bonus_insert
AFTER INSERT ON ITEM_VENDA
FOR EACH ROW
BEGIN
    DECLARE V_ID_FUNC INT;
    DECLARE V_NOME_FUNC VARCHAR(50);
    DECLARE V_BONUS FLOAT;

    SELECT ID_VENDEDOR INTO V_ID_FUNC FROM VENDA WHERE ID_VENDA = NEW.ID_VENDA;
    SELECT NOME INTO V_NOME_FUNC FROM FUNCIONARIO WHERE ID_FUNCIONARIO = V_ID_FUNC;
...
/*Adicionar cliente especial se gastar > R$500 (cashback 2%)*/
...
CREATE TRIGGER trg_cashback_cliente_insert
AFTER INSERT ON ITEM_VENDA
FOR EACH ROW
BEGIN
    DECLARE V_ID_CLIENTE INT;
    ...
    DECLARE V_SEXO ENUM('M','F','O');
    DECLARE V_IDADE INT;
    DECLARE V_CASHBACK FLOAT;

    SELECT ID_CLIENTE INTO V_ID_CLIENTE FROM VENDA WHERE ID_VENDA = NEW.ID_VENDA;
    SELECT NOME_CPADRAO, SEXO, IDADE INTO V_NOME, V_SEXO, V_IDADE
    FROM CLIENTE_PADRAO WHERE ID_CPADRAO = V_ID_CLIENTE;

    IF NEW.VALOR_TOTAL > 500 THEN
        SET V_CASHBACK = NEW.VALOR_TOTAL * 0.02;
        ...
        IF EXISTS (SELECT 1 FROM CLIENTE_ESPECIAL WHERE ID_CLIENTE = V_ID_CLIENTE) THEN
            ...
            WHERE ID_CLIENTE = V_ID_CLIENTE;
        ELSE
            INSERT INTO CLIENTE_ESPECIAL (ID_CESPECIAL, NOME_CESPECIAL, SEXO, IDADE, CASHBACK, ID_CLIENTE)
            VALUES (NULL, V_NOME, V_SEXO, V_IDADE, V_CASHBACK, V_ID_CLIENTE);
        END IF;
    END IF;
END $$
...
GROUP BY F.NOME
ORDER BY VENDAS_DO_PRODUTO DESC
LIMIT 1;

-- VALOR TOTAL GANHO COM O PRODUTO MAIS VENDIDO
SELECT 
    P.NOME_PRODUTO,
    SUM(I.VALOR_TOTAL) AS VALOR_GANHO
FROM ITEM_VENDA I
JOIN PRODUTO P ON I.ID_PRODUTO = P.IDPRODUTO
WHERE I.ID_PRODUTO = (
    SELECT ID_PRODUTO
...
-- ↓ Para o mês com menor venda do produto mais vendido:
SELECT 
    MONTH(V.DATA_VENDA) AS MES,
    SUM(I.QUANTIDADE) AS QUANTIDADE_VENDIDA
FROM ITEM_VENDA I
